...
def save_outputs_model(hparams, output_directory, index_line, max_nbr_lines, mel_outputs_postnet, au_outputs, alignments):
    ...
    output_file_path = os.path.join(output_directory, "TEST%04d_syn.WAVEGLOW" % index_line)
    ...
    os.makedirs(output_directory, exist_ok=True)
    output_file_path = os.path.join(output_directory, "TEST%04d_syn.AU" % index_line)
    ...
    output_file_path = os.path.join(output_directory, "TEST%04d_alignments.npy" % index_line)
...
def main():
    ...
    text_file_path = "filelists/GIPSA/test_AD_neutral.txt"  # Replace with the path to your text file
    checkpoint_path = "outputs_GIPSA/checkpoint_40000"
    output_directory = "outputs_GIPSA/inference/AV_Tacotron2_40000_test"
    ...
    if teacher_forcing:
        ...
        for line in lines:
            ...
            # Process or save the outputs as needed
            save_outputs_model(hparams, output_directory, index_line, len(lines), mel_outputs_postnet, au_outputs, alignments)
        print('Synthesis timer ends')
        toc()

    # Run shell commands Waveglow mel -> wav (specific to the vocoder used)
    run_shell_command(f"ls /research/crissp/lengletm/AV_Tacotron2/{output_directory}/*.WAVEGLOW > syn_waveglow.txt")
    run_shell_command("cd ../Tacotron2/waveglow && "
                        f"python3 inference.py -f /research/crissp/lengletm/AV_Tacotron2/syn_waveglow.txt -w waveglow_NEB.pt -o . -s 0.6 --denoiser")
    run_shell_command(f"mv ../Tacotron2/waveglow/*.wav {output_directory}")
